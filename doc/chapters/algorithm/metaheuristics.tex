...
Food energy $E_{solution}$ is computed using exponential function (equation \ref{equation:m_e_solution}). Two parameters are used: scaling factor $a$ and exponential base $q$. The exponent is inversely proportional to calibrated cost obtained as the best result from sampling phase. Selecting $a = \frac{1}{q}$ is recommended as it will scale the function down to unit scale. Selection of $q$ impacts amplification of cost quotient. Larger $q$ values will provide more energy $E_{solution}$ when a solution is better than sampled solution, giving more energy which can be used for further exploration.
