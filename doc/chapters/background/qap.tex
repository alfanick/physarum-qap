...
The discrete nature and the complexity of quadractic objective function causes problems in finding optimal solution thus making the problem NP-hard.
Horowitz and Sahni has proved in their book \cite{horowitz1978fundamentals}, that QAP needs an exponential time algorithm to solve optimally.
...
The first to transform the problem was Lawler, who represented it by an equivalent 0-1 linear programming problem and facilities the computations \cite{lawler1963}. He based his thinking on a traveling salesman problem looking for similirarities between these two \cite{charnsethikul1988exact}.
...
Kaufmann and Broeckx created another linearization, which has the smallest number of variables and constraints \cite{kaufman1978algorithm}.
...
Obtaining their mixed integer linear programming formulation could be achieved by replacint the product of binary variables by continuous variables as
\begin{equation}
  y_{ijpq} := x_{ij} x_{pq}
\end{equation}
...
It has similiraties to previous definition.
\begin{align}
  \text{minimize} &\null \sum_{i \in M} \sum_{j \in M} \sum_{p \in M} \sum_{q \in M} a_{ijpq}y_{ijpq} \\
  \text{subject to} &\null \sum_{i \in M} x_{ij} = 1, \; j \in M \\
  &\null \sum_{j \in M} x_{ij} = 1, \; i \in M \\
